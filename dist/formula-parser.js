...
function extractSheet(label) {
  if (typeof label !== 'string' || !SHEET_EXTRACT_REGEXP.test(label)) {
    return { label: label };
  }

  var _label$match = label.match(SHEET_EXTRACT_REGEXP),
      sheetQuoted = _label$match[1],
      localLabel = _label$match[2];

  var sheet = sheetQuoted.replace(/^'(.+)'$/, '$1');
  return { sheet: sheet, label: localLabel };
}
...
var Parser = function (_Emitter) {
  ...
  Parser.prototype._callCellValue = function _callCellValue(argLabel) {
    var _extractSheet = (0, _cell.extractSheet)(argLabel),
        label = _extractSheet.label,
        sheet = _extractSheet.sheet;
...
rules: [/^(?:\s+)/,/^(?:'(\\[']|[^'])*'![A-Za-z]+[0-9]+)/,/^(?:'(\\[']|[^'])*'!\$[A-Za-z]+\$[0-9]+)/,/^(?:'(\\[']|[^'])*'!\$[A-Za-z]+[0-9]+)/,/^(?:'(\\[']|[^'])*'![A-Za-z]+\$[0-9]+)/,/^(?:"(\\["]|[^"])*")/,/^(?:'(\\[']|[^'])*')/,/^(?:[A-Za-z]{1,}[A-Za-z_0-9\.]+(?=[(]))/,/^(?:#[A-Z0-9\/]+(!|\?)?)/,/^(?:.+!\$[A-Za-z]+\$[0-9]+)/,/^(?:.+!\$[A-Za-z]+[0-9]+)/,/^(?:.+![A-Za-z]+\$[0-9]+)/,/^(?:.+![A-Za-z]+[0-9]+)/,/^(?:\$[A-Za-z]+\$[0-9]+)/,/^(?:\$[A-Za-z]+[0-9]+)/,/^(?:[A-Za-z]+\$[0-9]+)/,/^(?:[A-Za-z]+[0-9]+)/,/^(?:[A-Za-z\.]+(?=[(]))/,/^(?:[A-Za-z]{1,}[A-Za-z_0-9]+)/,/^(?:[A-Za-z_]+)/,/^(?:[0-9]+)/,/^(?:\[(.*)?\])/,/^(?:&)/,/^(?: )/,/^(?:[.])/,/^(?::)/,/^(?:;)/,/^(?:,)/,/^(?:\*)/,/^(?:\/)/,/^(?:-)/,/^(?:\+)/,/^(?:\^)/,/^(?:\()/,/^(?:\))/,/^(?:>)/,/^(?:<)/,/^(?:NOT\b)/,/^(?:")/,/^(?:')/,/^(?:!)/,/^(?:=)/,/^(?:%)/,/^(?:[#])/,/^(?:$)/],
